...
export const SchoolPage: React.FC = () => {
  return (
    <div className="pt-24 pb-20 container mx-auto px-4 min-h-screen">
      ...
      {/* Header Section */}
      <div className="relative mb-16 bg-white p-8 md:p-12 rounded-[30px] border border-[#2563EB]/20 shadow-sm overflow-hidden">
        <div className="absolute top-0 right-0 w-40 h-40 bg-[#2563EB]/10 rounded-bl-full -mr-10 -mt-10"></div>
        <div className="relative z-10">
          <span className="inline-block px-4 py-1 bg-[#F97316] text-white text-sm font-bold mb-4 rounded-full">
            小学生対象
          </span>
          <h1 className="text-3xl md:text-5xl font-black text-[#1E293B] mb-6">
            サッカースクール<br/>
            <span className="text-[#2563EB] text-2xl md:text-3xl">浦崎式「個」の育成メソッド</span>
          </h1>
          <p className="text-[#1E293B]/80 text-lg leading-relaxed max-w-3xl mb-6">
            チーム練習だけでは補いきれない「個人の技術」「判断力」「身体操作」を徹底的に磨きます。<br/>
            少人数制だからこそできる、一人ひとりの癖や特徴に合わせた指導が特徴です。
          </p>
          <p className="text-[#1E293B]/70 text-sm leading-relaxed max-w-3xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
        {[
          ...
          <div key={idx} className="bg-white p-6 rounded-2xl border border-[#2563EB]/20 shadow-sm">
            <feature.icon className="w-10 h-10 text-[#2563EB] mb-4" />
            <h3 className="font-bold text-[#1E293B] mb-2">{feature.title}</h3>
            <p className="text-sm text-[#1E293B]/70">{feature.desc}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-20">
        {['技術 (Skill)', '戦術 (Tactics)', 'フィジカル (Physical)', 'メンタル (Mental)', '社会性 (Social)'].map((item, idx) => (
          <div key={idx} className="bg-white p-6 rounded-2xl border border-[#2563EB]/20 text-center shadow-[0_4px_0_#2563EB] hover:-translate-y-1 transition-transform">
            <div className="w-10 h-10 bg-[#FAFAFA] rounded-full mx-auto mb-3 flex items-center justify-center text-[#2563EB] font-bold">
              {idx + 1}
            </div>
            <h3 className="font-bold text-[#1E293B]">{item}</h3>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-20">
        <div className="bg-white p-8 rounded-[30px] shadow-sm relative">
           <TacticArrow className="absolute -top-6 -left-4 w-24 text-[#F97316]" />
           <h2 className="text-2xl font-bold text-[#1E293B] mb-6 flex items-center gap-3">
             <CalendarDays className="text-[#2563EB]" /> クラス・スケジュール
           </h2>
           
           <div className="space-y-6">
             <div className="border-b border-[#FAFAFA] pb-6">
               <div className="flex justify-between items-baseline mb-2">
                 <h3 className="text-xl font-black text-[#2563EB]">U-9 クラス</h3>
                 <span className="text-sm font-bold bg-[#FAFAFA] px-3 py-1 rounded-full text-[#1E293B]">小学1〜3年生</span>
               </div>
               <div className="flex items-center gap-2 text-[#1E293B] font-medium mb-2">
             ...
             <div>
               <div className="flex justify-between items-baseline mb-2">
                 <h3 className="text-xl font-black text-[#2563EB]">U-12 クラス</h3>
                 <span className="text-sm font-bold bg-[#FAFAFA] px-3 py-1 rounded-full text-[#1E293B]">小学4〜6年生</span>
               </div>
               <div className="flex items-center gap-2 text-[#1E293B] font-medium mb-2">
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-[#F97316] text-white p-8 rounded-[30px] shadow-lg relative overflow-hidden">
          ...
          <div className="bg-white p-8 rounded-[30px] shadow-sm flex-grow flex flex-col justify-center items-center text-center">
             <h3 className="text-lg font-bold text-[#1E293B] mb-4">まずは体験から！</h3>
             <p className="text-[#1E293B]/70 mb-6 text-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-20">
        {STAFF_MEMBERS.map((staff, idx) => (
          <div key={idx} className="bg-white p-6 rounded-2xl border border-[#2563EB]/20 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-start gap-4">
              <div className="w-16 h-16 bg-[#FAFAFA] rounded-full flex items-center justify-center flex-shrink-0">
                <User className="w-8 h-8 text-[#2563EB]" />
              </div>
              <div className="flex-grow">
                <span className="inline-block px-2 py-0.5 bg-[#2563EB]/10 text-[#2563EB] text-xs font-bold rounded mb-1">
                  {staff.role}
                </span>
                <h3 className="text-lg font-bold text-[#1E293B] mb-2">{staff.name}</h3>
                <p className="text-sm text-[#1E293B]/70 whitespace-pre-line leading-relaxed">
                  {staff.description}
                </p>
              </div>
            </div>
